...
const weweBaseUrl = normalizeBaseUrl(process.env.WEWE_RSS_BASE_URL || "");
const weweFeedIds = (process.env.WEWE_RSS_FEED_IDS || "")
  .split(",")
  .map(x => x.trim())
  .filter(Boolean);
const weweFeedFormatRaw = (process.env.WEWE_RSS_FEED_FORMAT || "rss").trim().toLowerCase();
const weweFeedFormat = allowedFeedFormats.has(weweFeedFormatRaw) ? weweFeedFormatRaw : "rss";
const weweSyncIntervalMinutes = Number(process.env.WEWE_SYNC_INTERVAL_MINUTES || "0");
const weweSyncOnStartup = parseBoolean(process.env.WEWE_SYNC_ON_STARTUP, false);
...
const buildWeweFeeds = (forceUpdate = false): FeedInput[] => {
  if (!weweConfigured) return [];
  return weweFeedIds.map(feedId => {
    const normalizedFeedId = feedId.toLowerCase();
    const path = normalizedFeedId === "all"
      ? `/feeds/all.${weweFeedFormat}`
      : `/feeds/${encodeURIComponent(feedId)}.${weweFeedFormat}`;
    const url = `${weweBaseUrl}${path}`;
    return {
...
app.get("/api/wewe/status", (req, res) => {
  res.json({
    configured: weweConfigured,
    running: weweSyncRunning,
    baseUrl: weweBaseUrl || null,
    feedIds: weweFeedIds,
    feedFormat: weweFeedFormat,
    syncIntervalMinutes: weweSyncIntervalMinutes,
...
app.post("/api/wewe/sync", async (req, res) => {
  if (!weweConfigured) return res.status(400).json({ error: "wewe_not_configured" });
...
if (weweConfigured && Number.isFinite(weweSyncIntervalMinutes) && weweSyncIntervalMinutes > 0) {
  const ms = Math.floor(weweSyncIntervalMinutes * 60_000);
...
if (weweConfigured && weweSyncOnStartup) {
  void runWeweSync(false).catch(() => {
  });
}
...
app.listen(port, () => {
  console.log("server started", port, {
    weweConfigured,
    weweFeedIds: weweFeedIds.length,
